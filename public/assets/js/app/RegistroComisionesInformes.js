...
class RegistroComisionesInformes {
    ...
    viewMensajeErrorCampos(campo_error, mensaje) {
        ...
        switch (campo_error) {
            case 'fecha_recibida':
                mensaje_error = document.getElementById('error-fecha_recibida');
            ...
            case 'cargo':
                mensaje_error = document.getElementById('error-cargo');
                mensaje_error.innerText = mensaje;
                break;

            case 'fecha_de_la_comision':
                mensaje_error = document.getElementById('error-fecha_de_la_comision');
                mensaje_error.innerText = mensaje;
                break;
    ...
    clearInputsForm() {
        ...
        input_form = document.getElementsByName('fecha_recibida');
        ...
        input_form = document.getElementsByName('cargo');
        input_form[0].value = '';

        input_form = document.getElementsByName('fecha_de_la_comision');
        input_form[0].value = '';
    ...
    imprimirView() {
        fetch(this.url + this.imprimir_url, {
            ...
            if (result instanceof Blob) {
                ...
                this.viewPopupMessage('error', 'Error', 'Ocurrio un error la obtener el vista previa de la impresion!');
...
let uri = "";
let uri_store = "/api/store-registro-comisiones-informes";
let uri_update = "/api/update-registro-comisiones-informes";
let uri_destroy = "/api/destroy-registro-comisiones-informes";
let uri_archivo = "/api/archivo-registro-comisiones-informes";
let uri_imprimir = "/api/imprimir-id-registro-comisiones-informes";
let uri_index = "/registro-comisiones-informes";

let registro_comisiones_informes = new RegistroComisionesInformes(container_popup, null, null, uri, uri_store, uri_update, uri_destroy, uri_archivo, uri_imprimir, uri_index);
...
onClick(document, 'click', '#btn-accion', e => {
    ...
    switch (accion) {
        case 'store':
            ...
            registro_comisiones_informes.setForm = form_data;
            registro_comisiones_informes.store();
            break;
...
onClick(document, 'click', '#delete', e => {
    ...
    registro_comisiones_informes.setForm = form_data;
    registro_comisiones_informes.setFilaTableDelete = fila;
    registro_comisiones_informes.confirmDestroy();
...
onClick(document, 'click', '#file', e => {
    ...
    form_data.append('id-reg', id);
    registro_comisiones_informes.setForm = form_data;
    registro_comisiones_informes.archivoApi();
...
onClick(document, 'click', '#print', e => {
    ...
    form_data.append('id-reg', id);
    registro_comisiones_informes.setForm = form_data;
    registro_comisiones_informes.imprimirView();
});
