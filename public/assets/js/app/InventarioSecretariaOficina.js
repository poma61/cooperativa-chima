...
class InventarioSecretariaOficina {
    ...
    viewMensajeErrorCampos(campo_error, mensaje) {
        ...
        switch (campo_error) {
            case 'detalle':
                mensaje_error = document.getElementById('error-detalle');
            ...
            case 'estado':
                mensaje_error = document.getElementById('error-estado');
                mensaje_error.innerText = mensaje;
                break;
    ...
    clearInputsForm() {
        ...
        input_form = document.getElementsByName('detalle');
        input_form[0].value = '';

        input_form = document.getElementsByName('cantidad');
        input_form[0].value = '';

        input_form = document.getElementsByName('estado');
        input_form[0].value = '';
    ...
    imprimirView() {
        fetch(this.url + this.imprimir_url, {
            ...
            if (result instanceof Blob) {
                ...
                this.viewPopupMessage('error', 'Error', 'Ocurrio un error al obtener la vista previa de la impresion!');
...
let uri = "";
let uri_store = "/api/store-inventario-secretaria-oficina";
let uri_update = "/api/update-inventario-secretaria-oficina";
let uri_destroy = "/api/destroy-inventario-secretaria-oficina";
let uri_archivo = "/api/archivo-inventario-secretaria-oficina";
let uri_imprimir = "/api/imprimir-id-inventario-secretaria-oficina";
let uri_index = "/inventario-secretaria-oficina";

let inventario_secretaria_of = new InventarioSecretariaOficina(container_popup, null, null, uri, uri_store, uri_update, uri_destroy, uri_archivo, uri_imprimir, uri_index);
...
onClick(document, 'click', '#btn-accion', e => {
    ...
    switch (accion) {
        case 'store':
            ...
            inventario_secretaria_of.setForm = form_data;
            inventario_secretaria_of.store();
            break;
...
onClick(document, 'click', '#delete', e => {
    ...
    inventario_secretaria_of.setForm = form_data;
    inventario_secretaria_of.setFilaTableDelete = fila;
    inventario_secretaria_of.confirmDestroy();
...
onClick(document, 'click', '#file', e => {
    ...
    form_data.append('id-reg', id);
    inventario_secretaria_of.setForm = form_data;
    inventario_secretaria_of.archivoApi();
...
onClick(document, 'click', '#print', e => {
    ...
    form_data.append('id-reg', id);
    inventario_secretaria_of.setForm = form_data;
    inventario_secretaria_of.imprimirView();
});
