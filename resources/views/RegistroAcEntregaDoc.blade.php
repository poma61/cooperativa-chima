...
<head>
    ...
    <link rel="stylesheet" href="{{asset('assets/css/sideBar.css')}}?v={{config('constants.VERSION_CSS')}}"
        type="text/css" media="all">
    <link rel="stylesheet" href="{{asset('assets/css/style500.css')}}?v={{config('constants.VERSION_CSS')}}" type="text/css" media="all">
...
<body>
    ...
    <div class="layout has-sidebar fixed-sidebar fixed-header">
        ...
        @include('components/layouts/fixed_header')
            ...
            <main class="content">
                <div class="title-info">Registro de Actas Entrega Documentacion</div>
                    ...
                    <div class="table-responsive">
                        <table id="tabla-datos" class="color-table-1">
                            ...
                            <tbody>
                                ...
                                $cantidad_filas=20;
                                @endphp
                                @foreach($registro_ac_entrega_doc as $row)
                                @if($cantidad_filas>0)
                                <tr>
                                    <td>{{$row->num_reg}}</td>
                                    ...
                                    <td>{{$row->institucion_responsable}}</td>
                                    <td>{{$row->observacion}}</td>

                                    <td>
                                        <a style="cursor:pointer;" id="file"
                                            data-id_registro="{{MyEncryption::encrypt($row->id)}}" class="file-reg"><i
                                    ...
                                    <td>
                                        <a href="{{route('route-frm-show-registro-ac-entrega-doc',MyEncryption::encrypt($row->id))}}"
                                            class="edit-reg"><i class="zmdi zmdi-edit"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <a href="{{route('route-pdf-id-registro-ac-entrega-doc',MyEncryption::encrypt($row->id))}}"
