...
use App\Models\RegistroAcLibroDeAlzas;
...
class ControllerRegistroAcLibroDeAlzas extends Controller
{
    /**
    ...
    {
        $data = RegistroAcLibroDeAlzas::where('status', true)->orderBy('fecha_emitida', 'DESC')->get();
        return view('RegistroAcLibroDeAlzas', ['registro_ac_libro_de_alzas' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $num_registro = str_pad(RegistroAcLibroDeAlzas::count('num_reg') + 1, 5, '0', STR_PAD_LEFT);
            //registrar historial del registro
            $data = [
                'table' => 'actas_libro_de_alzas',
                'descripcion' => 'Registro de Actas Libro De Alzas',
            ...
            $registro_ac_libro_de_alzas = new RegistroAcLibroDeAlzas($request->all());
            $registro_ac_libro_de_alzas->status = true;
            $blob = $request->file('archivo');
            $registro_ac_libro_de_alzas->archivo = $blob->openFile()->fread($blob->getSize());
            $registro_ac_libro_de_alzas->mime_type = $blob->getMimeType();
            $registro_ac_libro_de_alzas->id_usuario = Auth::user()->id;
            $registro_ac_libro_de_alzas->num_reg = $num_registro;
            $registro_ac_libro_de_alzas->save();
        } catch (Exception $ex) {
    ...
    public function show($id)
    {
        ...
        $data = [
            'registro_ac_libro_de_alzas' => $registro_ac_libro_de_alzas,
            'action' => 'update',
        ];

        return view('FrmRegistroAcLibroDeAlzas',  $data);
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($registro_ac_libro_de_alzas == null) {
                ...
                $validar['errors_db_messages'] = 'Registro de Actas Libro De Alzas null';
                return response()->json($validar);
            }
            ...
            $data = [
                ...
                'id_usuario' => Auth::user()->id,
                'type_query' => 'update',
                'registro' => $registro_ac_libro_de_alzas->num_reg,
                'status_register' => 'sucess',
            ];
            $history_db = new HistoryDB($data);
            $history_db->save();

            $registro_ac_libro_de_alzas->fill($request->all());
    ...
    public function destroy(Request $request)
    {
        ...
        try {
            ...
            if ($registro_ac_libro_de_alzas == null) {
                $respuesta = [
                    'errors_db' => true,
                    'errors_db_messages' => 'Registro de Actas Libro De Alzas null',
                ];
            ...
            $registro_ac_libro_de_alzas->status = false;
            $registro_ac_libro_de_alzas->save();
    ...
    {
        $registro_ac_libro_de_alzas = new RegistroAcLibroDeAlzas();
        $registro_ac_libro_de_alzas->id = 0;
        $registro_ac_libro_de_alzas->fecha_emitida = date('Y-m-d');
        $registro_ac_libro_de_alzas->simbolo = 'Gr.';
        $registro_ac_libro_de_alzas->peso_oro_fisico = '0.00';

        $num_registro = RegistroAcLibroDeAlzas::count('num_reg') + 1;
        $data = [
            'registro_ac_libro_de_alzas' => $registro_ac_libro_de_alzas,
    ...
    {
        try {
            $registro_ac_libro_de_alzas = RegistroAcLibroDeAlzas::where('status', true)->orderBy('fecha_emitida', 'DESC')->get();
        } catch (Exception $ex) {

            return view('PageNotFound', ['tipo_error' => 'DESCONOCIDO']);
        }

        $pdf = Pdf::loadView('PDFAllRegistroAcLibroDeAlzas', ['registro_ac_libro_de_alzas' => $registro_ac_libro_de_alzas]);
        ...
        return view('ViewPDFAllRegistroAcLibroDeAlzas', ['registro_ac_libro_de_alzas' => $registro_ac_libro_de_alzas, 'pdf' => $pdf->output()]);
    }
    ...
    public function pdfRegistrosAll()
    {
        ...
        return $pdf->download(date('d-m-Y') . '-ActasLibroDeAlzas.pdf');
    }
    ...
    public function imprimirRegistrosId(Request $request)
    {
        try {
            ...
            $pdf = Pdf::loadView('PDFIdRegistroAcLibroDeAlzas', ['registro_ac_libro_de_alzas' => $registro_ac_libro_de_alzas]);
            //tamaño carta array(0, 0, 612.00, 792.00)
            //tamaño oficio  array(0, 0, 612.00, 1008.00) o 'legal'
    ...
    public function pdfRegistrosId($id)
    {
        ...
        return $pdf->download(date('d-m-Y') . '-' . $registro_ac_libro_de_alzas->num_reg . '-ActasLibroDeAlzas.pdf');
    }
    ...
    public function archivoRegistroId(Request $request)
    {
        ...
        //de esta manera se devuelven solamente datos blob 
        return response()->streamDownload(function () use ($registro_ac_libro_de_alzas) {
            echo $registro_ac_libro_de_alzas->archivo;
        }, 'archivo', [
            "Content-Type" => $registro_ac_libro_de_alzas->mime_type,
        ]);
    }
    ...
    public function validateRequest(Request $request, $type)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'alza_de' => 'required|string|max:300',
            'peso_oro_fisico' => 'required|numeric',
            'simbolo' => 'required|string|max:150',
